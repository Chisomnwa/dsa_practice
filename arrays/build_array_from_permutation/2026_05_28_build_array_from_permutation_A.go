...
Step 2: Loop through nums using index i
	...
	The first iteration
		when i = 0, nums[0] = nums[nums[0]] → 0 → ans[0] = 0
		when i = 1, nums[1] = nums[nums[1]] → 2 → ans[1] = 1
		when i = 2, nums[2] = nums[nums[2]] → 1 → ans[2] = 2
		when i = 3, nums[3] = nums[nums[3]] → 5 → ans[3] = 4
		when i = 4, nums[4] = nums[nums[4]] → 3 → ans[4] = 5
		when i = 5, nums[5] = nums[nums[5]] → 4 → ans[5] = 3

step 3: Retuen array [0,1,2,4,5,3]
